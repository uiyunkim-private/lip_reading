...
weightcheckpoint_callback = tf.keras.callbacks.ModelCheckpoint(weight_file, verbose=1, save_weights_only=True, period=5)
...
model = BI_LSTM(nclasses=2,input_shape = input_shape)
model.compile(optimizer="Adam", loss="categorical_crossentropy", metrics=["accuracy"])
model = LoadLatestWeight(model, weight_dir)
model.fit(x=train_generator, epochs=5, callbacks=callback_list)
