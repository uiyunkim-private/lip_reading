...
class Network:
    ...
    def build(self):
        ...
        self.model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), loss=tf.keras.losses.categorical_crossentropy, metrics=["accuracy"])
    ...
    def fit(self,epochs=500):
        ...
        f.close()
        self.model.fit(x=self.train_generator,validation_data=self.validation_generator, epochs=3)
        self.model.save(os.path.join(MODEL_DIR,'resnet','model.h5'))


network = Network(name='test_model',batch_size=1)
